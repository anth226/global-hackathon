...
exports.getConversation = async (req, res, next) => {
  try {
    ...
    if (req.query.crypt === "decrypt" && req.body.private_key) {
      messages = getEncryptedMessages(messages, {
...
exports.newConversation = async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.public_key) {
      const receptor = await User.findById(req.params.recipient);
      ...
      const myPubkey = req.body.public_key;
      if (!recPubkey || !myPubkey) {
        return res.status(422).send({ error: "Technical issue." });
      }

      const body = crypto
        .publicEncrypt(recPubkey, Buffer.from(req.body.composedMessage))
        .toString("hex");
      const body_author = crypto
        .publicEncrypt(myPubkey, Buffer.from(req.body.composedMessage))
        .toString("hex");
      message = new Message({
        conversationId: newConversation._id,
        body,
    ...
    for (let key in io.sockets.sockets) {
      if (io.sockets.sockets.hasOwnProperty(key)) {
        if (
          utils.compareIds(req.params.recipient, io.sockets.sockets[key].userId)
        ) {
...
exports.sendReply = async (req, res, next) => {
  try {
    ...
    if (req.body.public_key) {
      const cv = await Conversation.findById(req.params.conversationId);
      const receptorId = cv.participants.filter(
        (pt) => !utils.compareIds(req.user._id, pt)
      )[0];
...
exports.updateMessage = async (req, res, next) => {
  try {
    if (req.body.public_key) {
      const message = await Message.findById(req.body).populate(
        "conversationId"
      );
      const receptorId = message.conversationId.participants.filter(
        (pt) => !utils.compareIds(req.user._id, pt)
      )[0];
      const receptor = await User.findById(receptorId);
      let pubRes = await axios.get(
      ...
      const body = crypto
        .publicEncrypt(recPubkey, Buffer.from(req.body.content))
        .toString("hex");
      const body_author = crypto
        .publicEncrypt(myPubkey, Buffer.from(req.body.content))
        .toString("hex");
    ...
    const sentReply = await Message.findById(req.body._id).populate({
      path: "conversationId author",
      select: "participants _id profile",
    });
    const io = sockets.io;
...
exports.createSuperMessage = function (myId, superId) {
  const conversation = new Conversation({
    participants: [myId, superId],
  });
  const greeting =
    "Welcome to #GLH2022 - you can use this message center to contact your host. You will be able to contact other participants once the event begins.";
  conversation.save((err, newConversation) => {
    if (err) {
      console.log(err);
    } else {
...
function getEncryptedMessages(messages, key) {
  let nmessages = [];
  for (let m of messages) {
    if (m.body_author) {
      let dcrContent = utils.compareIds(key.user_id, m.author._id)
        ? m.body_author
        : m.body;
      dcrContent = crypto
        .privateDecrypt(key.private_key, Buffer.from(dcrContent, "hex"))
        .toString();
      m.body = dcrContent;
    }
    nmessages.push(m);
